...
**Release verification completed on 2026-09-09.** R-01 through R-17 pass, the repository is public, and [Lettercast v1.0.0](https://github.com/iAmAhmad14/lettercast/releases/tag/v1.0.0) is published from tag `v1.0.0`. The release contains the intended extension ZIP and checksum file. A fresh download produced SHA-256 `2ce0db5ae36b93dd57a142f85455d1ad29fd203df265ed3901315ad28380121f`, exactly matching the published checksum and R-11 evidence, and the owner confirmed the downloaded extension works. The initial R-14 attempt proved that the real MV3 service-worker request omitted `Origin`, which the former exact-Origin admission policy rejected. ADR 0010 resolved that mismatch without adding identity or credentials. Production version `fd0cd7c3-dd6f-4cf4-bf85-7d6012f4a35c`, deployment `1083f8a8-5d16-41ea-88e2-c65fd656cdf0`, receives 100% traffic. A bounded production probe verified absent-Origin success, exact approved CORS, and rejection of incorrect supplied Origins. The repeated R-14 live smoke passed for Dune: Part Two, The Matrix, and Parasite using the exact R-11 artifact. R-15 verified graceful decline on unsupported and failing pages. R-16 verified ten live Dune portraits from only the approved TMDB `w185` HTTPS path and proved that one deliberately blocked image becomes a neutral local placeholder without disturbing the 99 native cast links or causing a CSP violation. The owner completed the full manual Chrome acceptance checklist against the exact artifact with no persistent errors. All disposable browser profiles and temporary harnesses were removed. Safe metadata retained the `TMDB_API_TOKEN` binding by name only and `CAST_RATE_LIMITER` at `60/60`.
